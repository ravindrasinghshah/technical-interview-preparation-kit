/**
 * cd to algos folder and then run "node duplicate.js"
   Find the left-most index of duplicate item.
 * Worst case complexity - O(log n)
 */

function bs_duplicate(nums, target) {
  let left = 0; let right = nums.length;
  while (left < right) {
    const mid = Math.floor((left + right) / 2);

    if (nums[mid] >= target) right = mid; // left most position of target; for right most remove equal to sign
    else left = mid + 1;
  }
  return left;
}
